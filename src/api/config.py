...
class LocalConfig(Resource):
    ...
    def post(self):
        ...
        if 'owner_short' in configuration:
            interface.getNode(node_id, False).setOwner(long_name=None, short_name=configuration['owner_short'])
        ...
        if 'config' in configuration:
            ...
            for section in configuration['config']:
                for pref in configuration['config'][section]:
                    meshtastic.setPref(localConfig, f"{section}.{pref}", str(configuration['config'][section][pref]))
        ...
        if 'module_config' in configuration:
            ...
            for section in configuration['module_config']:
                for pref in configuration['module_config'][section]:
                    meshtastic.setPref(moduleConfig, f"{section}.{pref}", str(configuration['module_config'][section][pref]))
                interface.getNode(node_id).writeConfig(section)
