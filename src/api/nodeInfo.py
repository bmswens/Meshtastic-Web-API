...
PositionModel = api.model("Position", {
...
UserModel = api.model("User", {
    "id": fields.String(
        example="!55c77c48",
        description="Predefined ID for the device."
    ),
    ...
    "shortName": fields.String(
        example="MD1",
        description="User defined short name of the device."
    ),
    "macaddr": fields.String(
        example="lLVVx3xI",
        description="Predfined mac address for th device."
    ),
    "hwModel": fields.String(
        example="TBEAM",
        description="predefined hardware model of th device."
    )
})

DeviceMetricsModel = api.model("Device Metrics", {
    ...
    "voltage": fields.Float(
        example=4.159,
        description="The current voltage of the device."
    ),
    "channelUtilization": fields.Float(
        example=5.4733334,
        description="Current channel utilization."
    ),
    "airUtilTx": fields.Float(
        example=3.154972,
        description="I'm not quite sure."
    )
})

NodeInfoModel = api.model("Node Info", {
    "num": fields.Integer(
        example=1439136840,
...
class NodeInfo(Resource):
    @api.doc(
        description="Returns all the known nodes as list."
    )
